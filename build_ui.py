...
.sub{color:var(--muted);max-width:62ch;margin:0 0 4px}
.conv{color:var(--muted);font-size:13px;max-width:70ch}
...
.glossary>div:target{border-color:var(--ink);background:#efe4c4;animation:gflash 1.3s ease}
@keyframes gflash{0%{background:#e6d48b}100%{background:#efe4c4}}
...
function buildRoster(){
  ...
  NAMES.forEach(name=>{
    ...
    b.innerHTML=`${name} <span class="g num">${DATA[name].goals}</span>`;
  ...
  selected.forEach(name=>{
    ...
    card.innerHTML=`
      <div class="top">
        ...
        <span class="meta num">${p.goals} goals · ${p.tournaments} World Cup${p.tournaments>1?'s':''}</span>
      </div>
